...
pm_handlers = require("./irchandlers.js").pm_handlers,
...
function createBot(host, room, cb) {
  ...
  bot.addListener('pm', function(nick, message) {
    ...
    // let's try to find a handler
    function tryHandler(i) {
      if (i >= pm_handlers.length) return;
      pm_handlers[i](host, nick, message, function(err, response) {
        if (err)
          return bot.say(nick, "oy, error: " + err);

        if (response != undefined) {
          bot.say(nick, response);
        } else {
          tryHandler(i+1);
        }
      });
